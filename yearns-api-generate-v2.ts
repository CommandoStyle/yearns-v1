 ...
 *
 * The two additions that matter for hybrid routing:
 *   1. Import selectModel + streamFromModel from model-router
 *   2. Replace the Anthropic stream block with streamFromModel()
 *
 * Everything else — auth, age gate, rate limiting, SSE formatting,
 * output filter, logging — is model-agnostic and unchanged.
 */
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  // ── Model selection — the only new line vs v1.0 ──────────────────────────
  const selectedModel = selectModel(body.explicitness)
  // selectedModel is 'CLAUDE' for levels 1-2, 'LLAMA_70B' or 'MIXTRAL' for 3-4.
  // Everything below is identical regardless.
  ...
  // 10–13. Stream, filter, log (background — doesn't block Response)
  ;(async () => {
    let fullText = ''

    try {
      // streamFromModel handles both Claude and Together.ai adapters
      for await (const token of streamFromModel(prompt, selectedModel)) {
        fullText += token
        controller.enqueue(encoder.encode(sseToken(token)))
      }

      // 11. Output filter (model-agnostic)
      ...
      // 12. Done event — includes which model was used (for analytics)
      controller.enqueue(
        encoder.encode(sseDone({
      ...
      // 13. Side effects (fire and forget)
