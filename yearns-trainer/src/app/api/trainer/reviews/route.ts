...
async function getQueue(trainerId: string): Promise<NextResponse> {
  ...
  // (Written there by the generation pipeline when blind_review=true)
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  // (DB computed column handles this, but we use it for the response)
  ...
  // If gold: trigger async corpus pipeline
  // (In production: send to a queue/edge function that processes gold stories)
  // For V1: a flag in the DB is enough — corpus export runs as a scheduled job
  if (corpusTag === 'gold') {
    // Fire-and-forget: update prompt_versions.sample_count + avg_score
    supabase.rpc('update_prompt_version_stats', {
      p_version: logEntry?.prompt_version,
    }).then().catch(console.error)
  }

  // Pre-load next story for seamless UX (trainer doesn't wait between stories)
  const nextQueue = await getQueue(trainerId)
  const nextData = await nextQueue.json()
...
async function computeSessionStats(
  ...
  return {
    ...
    top_failure_this_session:   topFailure as any,
  }
}
